...
export const useSamples = (audioContext = null, masterGainNode = null) => {
  ...
  const initializeAudioContext = useCallback(() => {
    // Use shared context if available, otherwise show warning
    if (audioContextRef.current) {
      return audioContextRef.current;
    }
    
    console.warn('Samples: No shared audio context provided. Audio may not work correctly.');
    return null;
  }, []);
  ...
  const playSample = useCallback(async (sample, options = {}) => {
    ...
    if (!audioContext) {
      console.error('Samples: AudioContext not available');
      return null;
    }
    
    // Check if AudioContext is suspended
    if (audioContext.state === 'suspended') {
      console.warn('Samples: AudioContext is suspended. Audio playback blocked by browser autoplay policy.');
      return null;
    }
    ...
    try {
      ...
      // Validate parameters
      const validPlaybackRate = validateAudioParam(playbackRate, 0.25, 4.0);
      const validVolume = validateAudioParam(volume, 0, 1);
      const validStartTime = validateAudioParam(startTime, 0, loadedSample.buffer.duration);
      const validWhen = validateTime(when || audioContext.currentTime, audioContext);
      
      source.playbackRate.value = validPlaybackRate;
      ...
      source.start(validWhen, validStartTime);
      ...
      return {
        ...
        stop: (when = audioContext.currentTime) => {
          const validStopTime = validateTime(when, audioContext);
          source.stop(validStopTime);
          currentlyPlayingRef.current.delete(playId);
        },
        fadeOut: (duration = 1.0) => {
          const validDuration = validateAudioParam(duration, 0.1, 10);
          const now = audioContext.currentTime;
          gainNode.gain.exponentialRampToValueAtTime(0.001, now + validDuration);
  ...
  }, [initializeAudioContext, loadSample, validateAudioParam, validateTime]);
